...
#             'raita_salad': 0,
#             'drinks': 0,
#             'mineral': 0  # Added mineral category
#         }
...
#         fg_color = "white" if self.is_dark_mode else "chocolate3"
#         button_bg = "red4" if self.is_dark_mode else "white"
#         button_fg = "white" if self.is_dark_mode else "black"
#         self.root.configure(bg=bg_color)
#         for widget in [self.heading_label, self.name_entry.master, self.phone_entry.master, self.bill_number_entry.master,
#                        self.textarea.master, self.total_entries['pulao'].master] + [item.entry.master for category in self.menu_items for item in self.menu_items[category]]:
#             widget.configure(bg='chocolate3' if not self.is_dark_mode else "black", fg=fg_color)
#         for button in self.total_entries['pulao'].master.winfo_children():
#             if isinstance(button, Button):
#                 button.configure(bg='chocolate3' if not self.is_dark_mode else "black", fg=fg_color)
#         self.total_entries['pulao'].master.winfo_children()[-1].configure(bg=button_bg, fg=button_fg)
